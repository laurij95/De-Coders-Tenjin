...
class Courses extends StatelessWidget{
 ...
 Courses({this.Email});
  ...
  Future<List>getCourses() async{
     ...
     final response = await http.post("http://192.168.1.10/tenjindb/getcourses.php",
        body:{
          "Email":Email,
        });
           ...
           child: new GestureDetector(
             onTap: () => Navigator.of(context).push(
               new MaterialPageRoute(
                 builder: (BuildContext context) => new CourseDetails(list: list, index: i,)
               )
             ),
             child: new Card(
               child: new ListTile(
               ...
               //  subtitle: new Text("Lecturer : ${list[i]['userid']}"),
               ),
             ),
           )
        );
